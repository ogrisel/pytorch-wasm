...
const URL = `${BASE}/notebooks/index.html?path=torch_mlp_demo.ipynb`;
const SHOTS = '/workspace/prototypes/emscripten-forge/torch-wasm/logs';
const LOGF = `${SHOTS}/44-torch-run.log`;
...
(async () => {
  ...
  const deadline = Date.now() + 420000; let last='';
  while (Date.now() < deadline) {
    ...
    if (/loss decreased/i.test(state)){ line('*** TORCH SUCCESS ***'); break; }
    if (/Traceback|RuntimeError|ImportError|ModuleNotFound|abort\(|Aborted|what\(\)|c10::/i.test(state)){ line('*** ERROR in cell ***'); break; }
  }
  await page.screenshot({ path: `${SHOTS}/pw-07-torch.png`, fullPage: true });
